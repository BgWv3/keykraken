...
from utils.scenario_runner import run_multiple_scenarios
...
def run_multiple_scenarios_ui():
    file_paths = filedialog.askopenfilenames(
        title="Select Scenarios",
        filetypes=[("JSON Files", "*.json")]
    )
    if file_paths:
        run_multiple_scenarios(file_paths, _automation_thread_func)
...
def setup_gui(app_root):
    ...
    ttk.Separator(top_frame, orient=tk.VERTICAL).pack(side=tk.LEFT, fill=tk.Y, padx=10, pady=5)
    # -- Run Multiple Scenarios Button --
    ttk.Button(top_frame, text="Run Multiple Scenarios", command=run_multiple_scenarios_ui).pack(side=tk.LEFT, padx=2)
